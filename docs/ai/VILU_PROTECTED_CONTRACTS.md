...
**A `srcset`+`sizes` approach on the gallery thumbnails (`vilu-residence-couple-sunset-beach-maldives.jpg` and `vilu-residence-maamigili-island-buggy-tour.jpg`) was tried and reverted (commits `4605344` then `f264aa3`) — do not re-attempt this without first understanding why it failed.** Live-browser testing found the browser's own `srcset`-candidate-selection-and-fetch mechanism intermittently left the `<img>` in a genuinely "broken" state (confirmed via `drawImage` throwing "the HTMLImageElement provided is in the 'broken' state") for the smaller candidate — even though the exact identical file loaded perfectly every time via a plain `<img src>` (confirmed both by direct browser navigation and a bare `new Image()` test), and `curl` confirmed the file itself was served correctly with the right `Content-Type`/byte count. The automated test suite (`phase12-preservation.test.js`) does NOT catch this class of bug — only real, live-browser loading verified it. If this is ever revisited, root-cause the actual failure first (candidate CDN/cache interaction? a Chrome bug specific to this file/CDN combination?) before shipping to real users again — a broken image icon on the live gallery is a worse outcome than the extra bytes.
...
**New major finding (not fixed): the homepage's Firestore SDK maintains a continuous WebChannel "Listen/channel" long-polling connection** — confirmed via real network-request timestamps (~60-80 tiny requests per page load, present even in the very first pre-fix baseline run, so not something this session's changes caused). This correlates with the largest remaining mobile long tasks (up to ~5.7s observed in one run). Confirmed via full-text search that the app's own code never calls `.onSnapshot()` anywhere in `vilu-website.html` — every Firestore call is a one-time `.get()`/`.set()`/`.runTransaction()`. This is inherent Firestore Web SDK transport behavior (the SDK opens this stream as part of its own architecture, independent of whether the app uses real-time listeners), not a custom application bug. Any fix (transport-level tuning, or further deferring Firebase init) touches the explicitly protected `ensureFirebaseReady()`/Firebase-loading architecture — do not attempt a fix without a dedicated, carefully-tested investigation and explicit owner authorization for that specific work, separate from general Phase 21 image/script tuning.
