...
## Authentication / security protected contracts (added 2026-09-06, Phase 47 audit)

Full architecture map and 7 classified findings (V1-V7) live in `VILU_COMPLETION_MATRIX.md` Phase 47 and that day's Phase 47 audit report — read them before touching any authentication code in `vilu-unified.html` or `vilu-agency-portal.html`. Key contracts until that phase is explicitly authorized for implementation:
...
- `migrateLegacyUser()` (`vilu-unified.html`) and `migrateLegacyAgency()` (`vilu-agency-portal.html`) are one-time, self-deleting legacy-credential fallbacks — do not delete either until the owner/an admin has confirmed (via Firebase Console, never via a full Auth export) that the corresponding accounts have already migrated to real Firebase Auth credentials. Deleting them first risks locking out a real account.
- Several code paths were traced and confirmed **dead** (not merely deprecated) this audit: the login-lockout/attempt-tracking/audit-log system (`vilu-unified.html:10604-10690+`, never called from the real login path), the embedded legacy agency mini-app and its second `routeByRole('agency')` routing path (both in `vilu-unified.html`), and the `vilu_agency_session`-based session-restore vestige (`vilu-agency-portal.html:1981`, confirmed never written by any current code). None of these are exploitable as currently wired, but none should be assumed load-bearing either — verify before touching.
...
- Never run a full Firebase Auth export (`firebase auth:export` or equivalent) as part of an audit — that pulls real password hashes into a local file, which is a bigger and riskier action than read-only architecture research warrants. A yes/no "does this account exist" question belongs to the owner/an authorized admin checking the Firebase Console directly.
