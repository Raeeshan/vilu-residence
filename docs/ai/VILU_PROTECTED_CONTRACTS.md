...
- `migrateLegacyUser()` (`vilu-unified.html`) and `migrateLegacyAgency()` (`vilu-agency-portal.html`) are one-time, self-deleting legacy-credential fallbacks (V1/V2) — do not delete either until the owner/an admin has confirmed (via Firebase Console, never via a full Auth export) that the corresponding accounts have already migrated to real Firebase Auth credentials. Deleting them first risks locking out a real account.
- The `password` field still written into `vilu_users`/`vilu_users_sync`/`vilu_agency_users` localStorage keys and the `USERS_UPDATE` BroadcastChannel payload — NOT yet sanitized out. Doing so before a full account-migration inventory risks silently breaking a real, not-yet-migrated staff/agency account's ability to self-heal into Firebase Auth. Sanitize only after the migration-status check above.
