...
**Status:** CURRENT — audit complete, minimal verified-safe cleanup implemented 2026-09-06 (commit `080874b`, feature branch only, NOT deployed) · **Owner approval required:** YES for the remaining items · **Production impact:** NONE (preview/feature-branch only)
Full architecture map + 7 classified findings (V1-V7) on record in `VILU_COMPLETION_MATRIX.md` Phase 47. **Implemented**: removed the fully dead/never-called login-lockout+session-timeout system, removed a dead unauthenticated agency-session-restore vestige, removed a fully dead embedded legacy agency mini-login, and closed a login-form enumeration signal — all four verified zero-caller/zero-behavior-change before removal, full test suite re-passed, both PMS/Agency Portal pages re-verified to load cleanly. **Intentionally deferred**: retiring the self-healing legacy credential fallbacks and sanitizing password fields out of localStorage/BroadcastChannel — both require confirming, via an owner/admin Firebase Console check (not an Auth export), which real accounts have already migrated; doing this blind risks locking out a real, currently-working staff/agency account. One dead code path (a second agency-routing branch) was traced but intentionally left in place since removing it means touching a function that runs on every single login for a purely cosmetic gain. No destructive security rewrites, no PMS/booking/pricing/Firestore-rules changes.
