...
**Status:** CURRENT — minimal cleanup implemented 2026-09-06 (commit `080874b`); remaining retirement work formally STOPPED with evidence, feature branch only, NOT deployed · **Owner approval required:** YES to ever revisit the stopped items · **Production impact:** NONE (preview/feature-branch only)
Full architecture map + 7 classified findings (V1-V7) on record in `VILU_COMPLETION_MATRIX.md` Phase 47. **Implemented**: removed the fully dead/never-called login-lockout+session-timeout system, removed a dead unauthenticated agency-session-restore vestige, removed a fully dead embedded legacy agency mini-login, and closed a login-form enumeration signal — all four verified zero-caller/zero-behavior-change before removal, full test suite re-passed, both PMS/Agency Portal pages re-verified to load cleanly. **Formally stopped, not merely deferred**: the owner checked Firebase Console (2026-09-06) and found `viluresidence@gmail.com` already migrated but `agency@viluresidence.com` has not — per the owner's own rule, this means `migrateLegacyUser()`/`migrateLegacyAgency()`/`DEFAULTS` and the localStorage/BroadcastChannel `password` field must all stay exactly as they are until that account (or whoever uses that role) completes a real login and migrates. One dead code path (a second agency-routing branch) was traced but intentionally left in place since removing it means touching a function that runs on every single login for a purely cosmetic gain. No destructive security rewrites, no PMS/booking/pricing/Firestore-rules changes.
