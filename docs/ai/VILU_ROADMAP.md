...
**Status:** IN PROGRESS — real baseline recorded, one confirmed fix implemented (commit `2aa4d04`, preview only), 2026-09-06 · **Owner approval required:** NO for measurement/this fix; YES for the deferred image-optimization pass and for production deployment · **Production impact:** NONE YET
Real Lighthouse baseline captured against production (mobile + desktop, multiple runs). Root-caused and fixed the single largest forced-reflow source on the page (Descent scroll-progress module reading layout too early, before the page settled) — deferred to `load`, zero visual change, confirmed via repeated before/after measurement. A ~782 KiB image-delivery opportunity (non-hero content images oversized for their display size) was identified but not implemented this pass — needs correct target dimensions across the full responsive matrix plus visual QA. Hero video re-encoding was evaluated and rejected: the local encoder (libopenh264, no perceptual rate control) can't safely shrink either file further without risking visible quality loss, which the owner explicitly forbids. Consent/analytics/theme script loading order was audited and left unchanged — real measured scripting cost is a few ms each; the "blocking time" Lighthouse reports is mostly network/parse wait, not something `defer`/`async` would fix, and touching load order risks the protected consent-before-analytics guarantee. Builds on and fully preserves the existing `ensureFirebaseReady()` lazy-init architecture (Phase 13B-2, already shipped — see `VILU_CHANGELOG.md`).
