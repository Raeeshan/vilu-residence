...
**Status:** IN PROGRESS — baseline + reflow fix + one verified-safe image fix shipped, one major new bottleneck documented, 2026-09-06 · **Owner approval required:** NO for measurement/fixes made; YES for a future Firestore-transport investigation and for production deployment · **Production impact:** NONE YET
Real Lighthouse baseline captured against production (mobile + desktop, multiple runs). Root-caused and fixed the single largest forced-reflow source on the page (Descent scroll-progress module reading layout too early) — deferred to `load`, zero visual change, confirmed via repeated before/after measurement (commit `2aa4d04`). Individually audited all 5 flagged oversized images across every homepage usage context; shipped one real, verified-safe fix (an Experiences-chip thumbnail, 109KB→10KB); attempted a `srcset`-based fix on 2 gallery thumbnails but found via live-browser testing a genuine, reproducible browser bug in the srcset-candidate-fetch path (the image loads perfectly via plain `src` but intermittently comes back "broken" via `srcset` selection) — reverted both rather than ship a real risk of a broken image on the live gallery (commits `4605344`, `f264aa3`). **New major finding**: the homepage's Firestore SDK maintains a continuous WebChannel long-polling connection (inherent SDK transport behavior, not an application bug — confirmed the app's own code only ever does one-time `.get()` reads) that correlates with the largest remaining mobile long tasks; any fix touches the explicitly protected Firebase-loading architecture, so it's documented for a dedicated future phase rather than touched here. Hero video re-encoding re-evaluated and rejected again (same encoder-limitation reasoning). Consent/analytics/theme script loading order re-confirmed untouched. Builds on and fully preserves the existing `ensureFirebaseReady()` lazy-init architecture (Phase 13B-2, already shipped — see `VILU_CHANGELOG.md`).
