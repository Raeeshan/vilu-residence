...
2. **Phase 47 (Security Hardening) is CURRENT, with a minimal verified-safe cleanup already implemented** (commit `080874b`, feature branch only, not deployed): the fully dead login-lockout/session-timeout system, a dead unauthenticated agency-session-restore path, and a fully dead embedded legacy agency mini-login were removed (all zero-caller, zero-behavior-change, re-verified via the full test suite plus live page loads with zero new console errors); a login-form enumeration signal was closed. **Deliberately NOT done**: retiring the self-healing legacy credential fallbacks (`migrateLegacyUser`/`migrateLegacyAgency`) or sanitizing `password` fields out of localStorage/BroadcastChannel — both require an owner/admin Firebase Console check (never a full Auth export) to confirm which real accounts have already migrated; doing it blind risks locking out a real, currently-working staff/agency account, which would be worse than leaving the current fallback in place. Full detail in `VILU_COMPLETION_MATRIX.md` Phase 47.
3. **No PMS/booking/pricing/Firestore-rules logic was changed** — only confirmed-dead code removed and one user-facing error string normalized. Not deployed to production; do not deploy without a fresh, explicit owner authorization, and do not begin the remaining Phase 47 items (V1/V2 retirement, storage sanitization) without owner sign-off on the Firebase Console check first.
...
Phase 47 (Security Hardening)'s minimal, verified-safe cleanup is done (commit `080874b`) — **the remaining items need one owner/admin action first**: confirm via Firebase Console (never a data export) whether the two default admin/agency accounts have already migrated to real Firebase Auth credentials. That answer determines whether `migrateLegacyUser()`/`migrateLegacyAgency()` and the DEFAULTS array can be safely removed, and whether/how localStorage-password sanitization can proceed without risking a real account lockout. Do not implement those remaining items without that check plus explicit owner authorization. Separately, Phase 21 (Performance) should start with an actual Lighthouse/CWV baseline (none has ever been run) and Phase 17 (weather visual prototype) is next in the roadmap's own numeric sequence and fully unblocked — neither is authorized to start by either audit alone; each needs its own explicit owner go-ahead per the standing workflow (see `VILU_DECISIONS.md` §"Standing workflow").
