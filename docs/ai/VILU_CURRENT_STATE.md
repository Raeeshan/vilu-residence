...
Both active workstreams (visual redesign, security hardening) are shipped and verified. Retiring `migrateLegacyUser()`/`migrateLegacyAgency()` themselves is **not** on the todo list — they're intentionally permanent by design, not a follow-up task. Per the master audit's recommended execution order (still current): Phase 21 (Performance) should start with an actual Lighthouse/CWV baseline (none has ever been run) and Phase 17 (weather visual prototype) is next in the roadmap's own numeric sequence and fully unblocked — neither is authorized to start by any prior audit alone; each needs its own explicit owner go-ahead per the standing workflow (see `VILU_DECISIONS.md` §"Standing workflow"). Phases 48/49 (PMS/Agency Portal hardening) have precisely-itemized remaining scope (monitoring, rate-limiting, auditability, lifecycle) in `VILU_COMPLETION_MATRIX.md` if the owner wants to pursue them next.
