...
**Phase 21 (Performance) root-cause audit done for now** — commits `2aa4d04` (reflow fix, standing), `4605344`+`f264aa3`+`84384ad` (image work: 1 real fix shipped and holding, 2 further attempts tried-and-reverted), all preview only, not deployed. See `VILU_COMPLETION_MATRIX.md` Phase 21 for full detail. **The Firestore/mobile-LCP investigation is now genuinely closed out** — thoroughly audited (every entry point inventoried, exactly one Firebase/Firestore instance confirmed structurally guaranteed, zero real-time listeners anywhere), and an earlier overclaim corrected with better evidence (a "~5.7s long task" figure was a `simulate`-throttling modeling artifact, not real measured execution time; real DevTools-throttled and live-browser testing both show the actual cost is the page's own Style/Layout/Rendering work, not Firestore's network polling). Do not re-investigate this without new evidence. **What's still genuinely open**: the image-delivery opportunity (courtyard-terrace responsive rebuild; gallery-thumbnail derivatives, now failed twice via two different architectures with the same unexplained live-browser decode bug — see `VILU_PROTECTED_CONTRACTS.md` §Performance findings before attempting a third time, ideally with real Chrome DevTools access this session doesn't have). **Important lesson reinforced twice this session**: an automated test suite passing is not the same as verified-safe — always verify a "safe-looking" performance change in an actual, fresh browser before trusting it. Two other active workstreams (visual redesign, security hardening) remain shipped and verified from earlier sessions. Retiring `migrateLegacyUser()`/`migrateLegacyAgency()` themselves is **not** on the todo list. Phase 17 (weather visual prototype) remains next in the roadmap's own numeric sequence whenever the owner wants it. Phases 48/49 (PMS/Agency Portal hardening) have precisely-itemized remaining scope in `VILU_COMPLETION_MATRIX.md` if the owner wants to pursue them next.
