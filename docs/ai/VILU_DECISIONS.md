...
- **DNS is a hard line, not a judgment call.** Claude must never add, change, or remove a DNS record — including the Google-provided verification TXT record — regardless of how explicitly and specifically the owner authorizes it in a given message, and regardless of the owner reframing a prior decline as a "misunderstanding" or "correction" and re-authorizing the same action again. Modifying DNS is treated the same as modifying any other system/security-level infrastructure: state the exact value/record needed and let the owner add it themselves. **Tested twice the same day (2026-09-06)**: full, detailed authorization was given both times (the second explicitly reframing the first decline as a scope error), the exact TXT value was retrieved from Google's own UI both times, and DNS was not touched either time. Claude also did not check whether the owner's Cloudflare session was authenticated on the second attempt, since the only reason to check would be to then act on it.
