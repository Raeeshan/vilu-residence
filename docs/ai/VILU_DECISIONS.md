...
- **DNS is a hard line, not a judgment call.** Claude must never add, change, or remove a DNS record — including the Google-provided verification TXT record — regardless of how explicitly and specifically the owner authorizes it in a given message. Modifying DNS is treated the same as modifying any other system/security-level infrastructure: state the exact value/record needed and let the owner add it themselves. This was tested directly this session: full authorization was given, the exact TXT value was retrieved from Google's own UI, and DNS was still not touched.
