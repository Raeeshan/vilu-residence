...
**Fourth pass, same day (2026-09-06): the owner reported that DNS TXT verification had been completed manually and Search Console showed the Domain property as verified. This was checked directly rather than taken at face value — and verification has NOT actually succeeded.** Three independent checks all agree: (1) the Search Console property switcher still lists `viluresidence.net` under "Not verified" after a full page reload; (2) clicking "Verify" on the pending Domain property returned **"Ownership verification failed... We couldn't find your verification token in your domain's TXT records. We found these DNS TXT records instead: `hosting-site=viluresidence`"**; (3) two independent public DNS resolvers (Google's own `dns.google` and Cloudflare's own `cloudflare-dns.com`) both show only `hosting-site=viluresidence` for `viluresidence.net` — no `google-site-verification=` record exists in public DNS right now. Cloudflare's own resolver reflecting its own zone with no propagation delay makes a simple "not yet propagated" explanation unlikely — more probable causes are the record being added to the wrong zone/domain, a typo in the record value, the record not actually being saved, or the change being made in a different Cloudflare account/zone than the one that actually serves `viluresidence.net`. **Phase 22 remains CURRENT, not COMPLETE.** The owner should double-check the record was saved under the exact zone `viluresidence.net`, as a TXT record with Name `@` (or the zone apex) and the exact value below, and confirm it appears in Cloudflare's own DNS records list for that zone before reporting again.
