...
// ---------------------------------------------------------------------------
// Shadow-mode / dry-run validation (Phase R7). Pure comparison, no
// Firestore, no network, no writes -- NOT called by beds24ChannelRateSync
// or beds24OutboundWorker, and therefore never runs in production traffic.
// Its purpose is the pre-flight step this pass's own STOP-BEFORE-DEPLOYMENT
// requirement implies: before outbound_channel_rates_enabled is ever
// flipped true, an operator can pull a set of {date: observedRate} pairs
// from a live, read-only Beds24 calendar GET (the exact technique already
// used for the 2026-09-22 Rule<->price-slot proof) and diff them against
// what this module WOULD have computed and pushed, with zero risk of ever
// writing anything. A mismatch here is a signal to fix the canonical
// schedule or the channel rule before enabling the flag, not something
// this function itself resolves.
function shadowCompareChannelRate({ intervals, rule, observed }) {
  const dates = Object.keys(observed || {}).sort();
  const rows = dates.map((date) => {
    const computed = computeChannelRateForDate({ intervals, date, rule });
    const expected = computed ? computed.rate : null;
    const actual = observed[date];
    return { date, expected, observed: actual, match: expected != null && expected === actual };
  });
  const mismatches = rows.filter((r) => !r.match);
  return { rows, mismatches, allMatch: dates.length > 0 && mismatches.length === 0 };
}
