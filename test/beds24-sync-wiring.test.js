...
//     BEDS24_REFRESH_TOKEN / defineSecret) -- the whole reason "core" and
//     "ota" are split codebases.
...
//   - functions-ota/index.js's beds24OutboundWorker ignores non-job
//     ota_pushes docs, never marks success without calling the adapter,
//     and never contains the literal string "refreshToken" in a console.*
//     call (the one place a credential could leak into logs).
//
...
section('Case E — functions-ota beds24OutboundWorker: correct job filtering, no silent success, no credential leakage');
test('beds24OutboundWorker is registered on ota_pushes/{id} with the BEDS24_TOKEN secret declared', () => {
  assert.ok(OTA.includes("exports.beds24OutboundWorker = onDocumentCreated({ document: 'ota_pushes/{id}', secrets: [BEDS24_TOKEN] }"));
});
test('beds24OutboundWorker ignores any doc that is not type=beds24_calendar_push AND status=pending', () => {
  const m = OTA.match(/exports\.beds24OutboundWorker[\s\S]*?\n\}\);/);
  assert.ok(m);
...
test('beds24OutboundWorker distinguishes retryable vs non-retryable failures (checks e.retryable) and is bounded (a fixed attempt count, not an infinite loop)', () => {
  ...
  assert.ok(OTA.includes('BEDS24_RETRYABLE_ATTEMPTS'));
